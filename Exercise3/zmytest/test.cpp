...
{
// ******************* HASHMAP
string GetRandomString( unsigned int n)
{
...
void MenuUtenteHashMap()
{
    ...
    do
    {
        ...
        {
            cout<<"che tipo di hashmap? (1) int (2) double (3) string"<<endl;
            cin>>scelta;
            if(scelta == '1')
            {
            ...
            {
                int n = AskInt("Inserisci il numero di elementi: ");
                lasd::HashTableClsAdr<double> ht(n);
                
                default_random_engine genx(random_device{}());
                uniform_real_distribution<double> distx(-1000,1000);
        ...
        else // hash open address
        {
            ...
            {
                int n = AskInt("Inserisci il numero di elementi: ");
                lasd::HashTableOpnAdr<double> ht(n);
...
//*******************************************BINARY TREE
  ...
  void MenuVec(int type, int dim)
  {
    ...
    lasd::Vector<float> vecfloat(dim);
    lasd::Vector<string> vecstring(dim);
    const typename BinaryTree<int>::Node* node_int;
    const typename BinaryTree<float>::Node* node_float;
    const typename BinaryTree<string>::Node* node_string;
    if(type == 1)
      riempi(vecint, dim);
    if(type == 2)
      riempi(vecfloat, dim);
